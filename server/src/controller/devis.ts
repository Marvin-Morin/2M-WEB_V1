...
export const formulaireController = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    if (!name && !email && !phone) {
      res.status(400).json({
        message:
          "Veuillez remplir tous les champs obligatoires : nom, email, et téléphone.",
      });
    ...
    const selectedSiteTypes =
      Object.entries(site_type)
    ...
    const selectedpaymentOptions =
      Object.entries(payment_options)
        .filter(([key, value]) => value === true)
        .map(([key]) => key.replace(/-/g, " "))
        .join(", ") || "Non spécifié";
    ...
    const selecteddeliveryOptions =
      Object.entries(delivery_options)
        .filter(([key, value]) => value === true)
        .map(([key]) => key.replace(/-/g, " "))
        .join(", ") || "Non spécifié";
